...
//   }
  ...
  async initializePayment(amount: number, email: string, sponsorshipId: string) {
    try {
      const reference = uuidv4(); // Generate unique transaction reference
      ...
      if (!sponsorship) {
        throw new HttpException('Sponsorship request not found', HttpStatus.NOT_FOUND);
      }
  ...
  async verifyPayment(reference: string) {
    ...
    try {
      ...
      if (!response.data || !response.data.data) {
        ...
        throw new HttpException('Invalid response from Paystack', HttpStatus.BAD_REQUEST);
      }
      ...
      if (!existingDonation) {
        ...
        throw new HttpException('Transaction reference not found', HttpStatus.NOT_FOUND);
      }
  
      console.log('Updating donation record...');
  
      const paymentStatus =
        paymentData.status === 'success' ? PaymentStatus.SUCCESS : PaymentStatus.FAILED;
